...
export default function ContasPagar() {
  ...
  const hasNonStatusFormChanges = (conta: ContaPagar) => {
  ...
  const saveConta = async (event?: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (shouldSaveBaseData) {
        ...
        savedContaId = isEditing ? editingConta!.id : (result.data as ApiAccountsPayable).id;
      }

      if (!shouldSaveBaseData && isEditing && editingConta!.status === 'pago' && formData.status !== 'pago') {
              ...
              {paginationItems.map((item) =>
                typeof item === 'number' ? (
                  <Button
                    ...
                    size="sm"
                    className={item === pagination.page ? 'cursor-pointer bg-blue-600 hover:bg-blue-700 dark:bg-[#075985] dark:hover:bg-[#0e7490] dark:text-white' : 'cursor-pointer dark:border-[#3b4658] dark:bg-[#273447] dark:text-slate-200 dark:hover:bg-[#314155]'}
