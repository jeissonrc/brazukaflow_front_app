...
export default function ContasReceber() {
  ...
  const handleMassOriginTypeChange = (value: string) => {
    setMassSelectedOrigin(null);
    setMassForm((prev) => ({ ...prev, origemCliente: value === 'cliente' }));
  };
  ...
  const findMassOriginMatch = (originDescription: string) => {
    const sameContextOrigins = origins.filter((origin) => Number(origin.category) === 2 && isTrue(origin.person) === massForm.origemCliente);
    ...
    const differentTypeExact = origins.find(
      (origin) =>
        Number(origin.category) === 2 &&
        isTrue(origin.person) !== massForm.origemCliente &&
  ...
  const generateMassContas = async (payload: MassGenerationPayload, originId?: number) => {
    ...
    try {
      ...
      showNewestRecordsFirst();
      toast.success(`${payload.parcelas} conta(s) a receber gerada(s) com sucesso.`);
  ...
  const saveMassContas = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (
      massSelectedOrigin &&
      isTrue(massSelectedOrigin.person) === massForm.origemCliente &&
  ...
  const massOriginOptions = useMemo(() => {
    ...
    return origins
      .filter((origin) => Number(origin.category) === 2 && isOriginActive(origin) && isTrue(origin.person) === massForm.origemCliente)
      .filter((origin) => {
  ...
  }, [massForm.origemCliente, massOriginSearchTerm, origins]);
                                ...
                                {massOriginOptions.length === 0 ? (
                                  ...
                                  massOriginOptions.map((origin) => (
                                    ...
                                    >
                                      ...
                                      <span className="text-xs text-gray-500 dark:text-slate-400">Código {origin.id} - {isTrue(origin.person) ? 'Cliente' : 'Operação'}</span>
                                    </button>
                                  ))
                                )}
                              </div>
        ...
        <AlertDialogContent className="dark:border-[#2f394a] dark:bg-[#1f2a37] dark:text-slate-100">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {massOriginMatchKind === 'exact'
                ? 'Já existe uma origem cadastrada com este nome para contas a receber. Deseja reutilizá-la?'
                : massOriginMatchKind === 'different-type'
                  ? `Já existe uma origem com este nome cadastrada como tipo ${isTrue(matchedMassOrigin?.person) ? 'Cliente' : 'Operação'}. Ao usar esta origem, as contas serão geradas com ${isTrue(matchedMassOrigin?.person) ? 'Cliente' : 'Operação'}.`
                  : massOriginMatchKind === 'different-type-similar'
                    ? `Encontramos uma origem semelhante cadastrada como tipo ${isTrue(matchedMassOrigin?.person) ? 'Cliente' : 'Operação'}. Deseja usar esta origem ou criar uma nova?`
                  : 'Encontramos uma origem semelhante para contas a receber. Deseja usar esta origem ou criar uma nova?'}
            </AlertDialogDescription>
          </AlertDialogHeader>
          ...
          <div className="rounded-md border border-gray-200 bg-gray-50 p-4 dark:border-[#2f394a] dark:bg-[#273447]">
            <div className="grid gap-3 text-sm">
              ...
              <div>
                ...
                <p className="mt-1 text-gray-900 dark:text-slate-100">{isTrue(matchedMassOrigin?.person) ? 'Cliente' : 'Operação'}</p>
              </div>
            </div>
          </div>
